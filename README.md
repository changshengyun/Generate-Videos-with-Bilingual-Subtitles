...
- **当前版本**：`V5.0.0`（以 V4.4 精简基线进入 V5；本次仅同步版本、文档与发布分支，不改变 App 运行行为）
...
- **当前构建产物**：`app/build/outputs/apk/debug/app-debug.apk`（Debug 签名；发布构建必须使用 `-PenableWhisperNative=true assembleDebug`）。
...
方式二：本地版本化产物
  dist/LyricCaptioner-v5.0.0-debug.apk（构建后可复制至此，仅用于本地分发，dist/ 不入库）
...
- **版本定位**：`V5.0.0` 是基于 V4.4 精简代码基线建立的首个 V5 开发分支版本。
- **运行行为**：不新增、不删除产品功能；本次只同步版本号（`5.0.0 / 5000`）、README、项目状态和远程 `dev` 分支。
- **发布边界**：只推送 `dev`，不推送或改写 `main`，不创建 V5 标签。
...
> 说明：以上是 V4 阶段的历史证据，不代表本次 V5.0 重新完成真机验收。V4.4 精简时测试代码（`app/src/test/`、`app/src/androidTest/`）与沙箱代码已从仓库移出，可从 git 历史（`v4.4.0`）找回。
...
└─────────────┘          └──────────────────┘        │ 系统相册导入导出            │
                                                      ...
                                                      └───────────┬────────────────┘
                                                                  ▼
 V5.0 dev 发布（本次）←─ V4.4 收官/精简 ←─ V4.3.x 修复群 ←─ V4.3 ←─ V4.2 ←─ V4.1 ←─ V4 主链路
 ┌────────────────┐    ┌──────────────┐
 │版本号 5.0.0/5000│    │仓库整理/版本号 │
 │README/状态同步  │    │V4.4 标签与精简 │
 │建立远程 dev 分支│    └──────────────┘
 └────────────────┘
```
...
| **V5.0** | **建立 V5 开发基线** | 以 V4.4 精简代码为基础同步 `5.0.0 / 5000`、README 与项目状态，并建立远程 `dev` 分支；不改变 App 运行行为 |
