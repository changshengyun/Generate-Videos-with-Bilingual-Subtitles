...
/**
 * HTTP client for DeepSeek Responses API (with web_search tool) and Chat Completions API.
...
class ResponsesApiClient(
     ...
     * Returns the final text output and search action count.
     */
    suspend fun callResponses(
        apiKey: String,
        input: String,
        tools: List<Map<String, Any>> = listOf(mapOf("type" to "web_search")),
    ): ResponsesApiResult = withContext(Dispatchers.IO) {
    ...
    /**
     * Execute a Chat Completions call (for Flow 3 bilingual generation).
     */
    ...
    private fun providerFailure(
        ...
        CaptionEnhancementProviderException(kind, "Responses API request failed.", cause)

    private fun parseResponsesResult(body: String): ResponsesApiResult {
        ...
        for (item in output) {
            ...
            when (type) {
                ...
                "web_search_call" -> searchOutputs.add(obj.toString())
            }
        }

        return ResponsesApiResult(textOutput = textOutput, searchActions = searchOutputs)
    }
