...
class FfmpegKitSubtitleExporter(
    ...
    override suspend fun export(
        ...
        try {
            ...
            stage = "font_config"
            // Map font families to real (non-symlink) system font files. On OEM builds
            // (e.g. MIUI) most entries under /system/fonts are symlinks, which fontconfig
            // fails to scan, producing "fontselect: failed to find any fallback font" and
            // aborting the subtitles filter. Explicit mapping avoids that scan entirely.
            FFmpegKitConfig.setFontDirectory(
                appContext,
                "/system/fonts",
                mapOf(
                    "sans-serif" to "NotoSansCJK-Regular.ttc",
                    "serif" to "NotoSansCJK-Regular.ttc",
                    "monospace" to "NotoSansCJK-Regular.ttc",
                ),
            )
