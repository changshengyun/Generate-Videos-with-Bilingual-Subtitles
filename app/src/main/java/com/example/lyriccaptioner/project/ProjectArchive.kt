...
class ProjectArchive(
    ...
    fun write(snapshot: ProjectSnapshot): String = buildString {
        ...
        appendField("defaultBackgroundEnabled", defaultStyle.backgroundEnabled.toString())
        appendField("defaultBackgroundColorHex", encode(defaultStyle.backgroundColorHex))
        appendField("captions", encodeCaptionsV7(snapshot.captions))
    ...
    fun read(raw: String): ProjectSnapshot {
        ...
        return when {
            ...
            header == MAGIC_V7 -> readV7(normalized)
    ...
    /** v7 adds backward-compatible solid-background fields to defaults and cue overrides. */
    private fun readV7(raw: String): ProjectSnapshot {
        val (values, _) = splitHeader(raw)
        val legacyStyle = SubtitleStyle(
            fontSizeSp = values.optionalInt("fontSizeSp", 24),
            bottomMarginPercent = values.optionalInt("bottomMarginPercent", 12),
        ...
        return ProjectSnapshot(
            videoUri = values.decodeOptionalNullable("videoUri"),
            videoDurationMs = values.optionalLong("videoDurationMs", null),
            captions = decodeCaptionsV7(values["captions"], projectLayout),
            exportProfile = profile,
            captionProcessing = values.readCaptionProcessing(),
            captionLayout = projectLayout,
            defaultCaptionStyle = values.readDefaultCaptionStyleV7(),
        )
    }
    ...
    private fun encodeCaptionsV7(captions: List<CaptionCue>): String {
        val payload = captions.joinToString(RECORD_SEPARATOR) { cue ->
            val style = cue.styleOverride?.validated()
            val layout = cue.layoutOverride?.validated()?.takeUnless { it.isEmpty }
            listOf(
                ...
                style?.alignment?.name.orEmpty(),
                style?.outlineWidthRatio?.toString().orEmpty(),
                style?.backgroundEnabled?.toString().orEmpty(),
                style?.backgroundColorHex?.let(::encode).orEmpty(),
                (layout != null).toString(),
                layout?.xRatio?.toString().orEmpty(),
                layout?.yRatio?.toString().orEmpty(),
                layout?.widthRatio?.toString().orEmpty(),
    ...
    private fun decodeCaptionsV7(
        ...
        if (payload.isEmpty()) return emptyList()
        return payload.split(RECORD_SEPARATOR).mapIndexed { index, record ->
            val fields = record.split(FIELD_SEPARATOR, limit = 24)
            if (fields.size != 24) throw ProjectArchiveFormatException("Malformed subtitle record $index.")
            val hasStyle = fields[8].requiredBoolean("caption style override presence")
            val hasLayout = fields[20].requiredBoolean("caption layout override presence")
            val style = if (hasStyle) readCaptionStyleOverrideV7(fields, index) else {
                if (fields.subList(9, 20).any(String::isNotBlank)) {
                    throw ProjectArchiveFormatException("Caption style override $index is inconsistent.")
                }
                null
            }
            val layout = if (hasLayout) readCaptionLayoutOverrideV7(fields, index).also {
                if (it.isEmpty) throw ProjectArchiveFormatException("Caption layout override $index is empty.")
                runCatching { resolveCaptionLayout(projectLayout, it) }.getOrElse { error ->
                    throw ProjectArchiveFormatException("Caption layout override $index is out of bounds.", error)
                }
            } else {
                if (fields.subList(21, 24).any(String::isNotBlank)) {
                    throw ProjectArchiveFormatException("Caption layout override $index is inconsistent.")
                }
                null
            }
    ...
    private fun readCaptionStyleOverrideV7(fields: List<String>, index: Int): CaptionStyleOverride =
        CaptionStyleOverride(
            fontSizeRatio = fields[9].optionalStrictFontSizeRatio("caption $index override fontSizeRatio"),
            primaryColorHex = fields[10].decodeStrictColorOrNull("caption $index override primaryColorHex"),
            secondaryColorHex = fields[11].decodeStrictColorOrNull("caption $index override secondaryColorHex"),
            ...
            alignment = fields[16].optionalStrictEnum<CaptionAlignment>("caption $index override alignment"),
            outlineWidthRatio = fields[17].optionalStrictOutlineWidthRatio("caption $index override outlineWidthRatio"),
            backgroundEnabled = fields[18].optionalStrictBoolean("caption $index override backgroundEnabled"),
            backgroundColorHex = fields[19].decodeStrictColorOrNull("caption $index override backgroundColorHex"),
        )
    ...
    private fun readCaptionLayoutOverrideV7(fields: List<String>, index: Int): CaptionLayoutOverride =
        try {
            CaptionLayoutOverride(
                xRatio = fields[21].optionalStrictFloat("caption $index layout override xRatio"),
                yRatio = fields[22].optionalStrictFloat("caption $index layout override yRatio"),
                widthRatio = fields[23].optionalStrictFloat("caption $index layout override widthRatio"),
            )
        } catch (error: ProjectArchiveFormatException) {
            throw error
        } catch (error: IllegalArgumentException) {
    ...
    private fun Map<String, String>.readDefaultCaptionStyleV7(): DefaultCaptionStyle =
        readDefaultCaptionStyle().copy(
            backgroundEnabled = optionalBoolean("defaultBackgroundEnabled", false),
            backgroundColorHex = decodeStrictColor(
    ...
    private companion object {
        ...
        const val MAGIC_V7 = "# LyricCaptionerProject v7"
