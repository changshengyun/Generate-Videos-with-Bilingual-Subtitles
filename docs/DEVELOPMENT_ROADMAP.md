...
| `V3-EDITOR-002` | `PARTIAL_PASS / PER_CUE_STYLE_EDITOR_VERIFIED / PHYSICAL_DEVICE_UI_WAIVED_BY_USER`（R1 candidate） | R1 已修复纯位置覆盖清除、ASS 中文颜色及 Compose/ASS 坐标/anchor 语义；等待 Brain 重新验收，物理 UI 继续按用户授权豁免 |
...
一次只激活一个实施阶段。当前活动阶段为 `V3-EDITOR-002 / R1`，Developer 已回交候选 `PARTIAL_PASS / PER_CUE_STYLE_EDITOR_VERIFIED / PHYSICAL_DEVICE_UI_WAIVED_BY_USER`；完整 R1 验收矩阵只保存在 `CURRENT_TASK.md`，等待 Brain 重新验收。不得启动 `V3-AI-001`。
