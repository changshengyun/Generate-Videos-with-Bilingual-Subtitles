...
| `V3-EDITOR-002` | `PARTIAL_PASS / PER_CUE_STYLE_EDITOR_VERIFIED / PHYSICAL_DEVICE_UI_WAIVED_BY_USER` | R3 已完成源视频相对字号/描边、v6 迁移、pixel aspect ratio 与 Media3 FIT 截断复现；R2 的共享 FIT 主矩形、x/y/width 路径及 R1/v5 能力保持。等待 Brain 正式裁决，不启动歌词链路或真机 UI |
...
一次只激活一个实施阶段。当前活动阶段为 `V3-EDITOR-002 / R3`，R3 实现与完整回归已完成，Developer 候选为 `PARTIAL_PASS / PER_CUE_STYLE_EDITOR_VERIFIED / PHYSICAL_DEVICE_UI_WAIVED_BY_USER`，等待 Brain 正式裁决；完整 R3 验收矩阵与 Limbs 并行计划只保存在 `CURRENT_TASK.md`。不得启动 `V3-AI-001` 或真机 UI。
