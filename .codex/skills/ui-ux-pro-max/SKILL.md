...
## LyricCaptioner Project Guardrails

- This repository is a native Android Jetpack Compose + Material 3 app. Always use the `jetpack-compose` stack and ignore web, React, Tailwind, GSAP, SwiftUI, Flutter, and React Native implementation advice.
- Design guidance does not authorize dependency upgrades, Compose alpha APIs, navigation migrations, generated font downloads, architecture changes, or changes to the media/model pipeline.
- Treat `docs/DEVELOPMENT_ROADMAP.md`, `docs/CURRENT_TASK.md`, and `docs/PROJECT_STATE.md` as the only active state documents.
- Do not run `--persist` or create a `design-system/` directory unless the user explicitly authorizes an additional design-system artifact. Use search output transiently for the active UI task.
- Visual acceptance requires Pixel_8 emulator screenshots, safe-area checks, readable Chinese text, accessible touch targets, and preservation of the real local-video workflow.
