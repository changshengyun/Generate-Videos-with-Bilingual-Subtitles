...
- Use this skill to review safe areas, full-screen preview, IME behavior, status/navigation-bar legibility, and scrollable content in the existing Compose UI.
- Do not upgrade SDK/Compose dependencies, migrate navigation, or rewrite the Activity structure unless the active task explicitly authorizes it.
- Existing `statusBarsPadding()` and `navigationBarsPadding()` behavior is verified baseline evidence; change it only with a regression test and new Pixel_8 screenshots.
