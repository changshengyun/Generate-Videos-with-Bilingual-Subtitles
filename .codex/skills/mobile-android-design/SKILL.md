...
- Preserve the existing Kotlin, Jetpack Compose, Material 3, ViewModel, Media3, and FFmpegKit architecture.
- Recommendations such as dynamic color, Navigation Compose, adaptive libraries, or new icon/font packages are optional ideas, not authorization to add dependencies or change navigation.
- Optimize the phone editing workflow around video preview, local video import, recognition/translation status, subtitle editing, and export. Do not turn the product into a generic card dashboard.
- Use Chinese copy, 48dp minimum Android touch targets, semantic content descriptions, clear loading/error/empty states, and screenshot-based Pixel_8 acceptance.
- Read the three active project documents before implementation and remain inside the current module boundary.
