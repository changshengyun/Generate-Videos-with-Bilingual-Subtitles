...
| V2-E2E-002 | `USER_ACCEPTED / DEVICE_PATH_VERIFIED` | 用户已在个人 ARM64 手机完成人工主链路验收，并决定关闭 V2，不再追加自动化或补充证据操作 |

一次只实施一个模块。开发窗口可自行处理模块内普通 Bug；只有架构/技术栈变更、新大型依赖、破坏性数据操作、需求冲突或无法证明安全的删除，才交由决策窗口处理。

## V2 最终用户验收（2026-08-02）

- 用户在 `fcf4b0cb / 25098PN5AC / ARM64 / 1220x2656 / 520 dpi` 上手动验证了真实视频导入、本地 Whisper、本地 OPUS-MT、FFmpegKit 导出和 Media3 播放主链路。
- 用户确认 V2 已满足其当前使用要求并明确要求结束 V2，不再执行额外真机、恢复、重绑、源文件哈希或自动化补证。
- 英文识别“可用、较此前明显改善”仅记录为用户主观验收结论，不替代固定歌词数据集的 WER/CER 质量证明；`V2-ASR-002 / FIXTURE_REQUIRED` 的历史证据限制保留。
- 中文翻译可运行，但用户认为表达仍偏机器化；该问题转入 V3 产品与技术决策，不把 V2 翻译质量描述为最终产品标准。
- V2 最终状态：`USER_ACCEPTED / DEVICE_PATH_VERIFIED`。功能代码基线保持 `8a48d88`。

## 历史临时测试模式

自 2026-07-30 起，V2 后续模块曾执行 `SIMULATOR_ONLY_TEMPORARY`。该临时门禁已由用户后续真机人工验收结束；本节只保留为 V2 历史记录，不再参与 V3 调度。
