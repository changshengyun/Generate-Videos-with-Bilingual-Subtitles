# LyricCaptioner V3 产品与架构草案

## 文档状态

- 状态：`DRAFT / NOT_AUTHORIZED_FOR_IMPLEMENTATION`
- 日期：`2026-08-01`
- 目的：保存 V3 的产品方向、架构提案、阶段建议和决策门禁。
- 本文不是当前执行任务，不修改 V2 的三份活动文档、模块状态或设备门禁。
- V2 当前仍以 `docs/DEVELOPMENT_ROADMAP.md`、`docs/CURRENT_TASK.md` 和 `docs/PROJECT_STATE.md` 为唯一活动状态面。

## V3 核心目标

在保留 Android 本地视频编辑、字幕时间轴、Media3 预览和 FFmpegKit 导出的基础上，将中文翻译与歌词纠错从本地 OPUS-MT 改为云端结构化 AI 增强：
...
V3 提升的是“最终字幕质量”，不能在没有固定评测数据时表述为“ASR 模型准确率已经提高”。
...
| 阶段 | 目标 | 主要验收 |
|---|---|---|
| `V3-DEC-001` | 冻结 API、隐私、歌词来源、离线策略和 ASR 模型决策 | 用户批准架构提案和回滚方案 |
...
一次只激活一个阶段。`V3-DEC-001` 未通过前，不实施生产 API、依赖、权限、模型或打包变化。
...
1. 大模型/API 供应商与结构化输出能力。
2. 是否建设自有后端，以及认证、配额和费用承担方式。
3. 用户字幕是否允许上传、保留多久，以及隐私提示形式。
4. 歌曲识别和歌词来源：用户输入、音频指纹、授权服务，或仅作无保证的 AI 推断。
5. 断网时是否允许只保留英文 ASR 并继续编辑/导出。
6. 默认继续使用当前 Whisper，还是批准大型候选模型的独立下载与评测。
